  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value) {
        ...
        _push(`<!--]--></div><div id="projectIndexText" class="relative h-full w-full DM-Mono leading-tight flex flex-col leading-normal col-start-9 col-span-16"><!--[-->`);
        ssrRenderList(myData.value, (project, index) => {
          _push(`<div class="${ssrRenderClass([{ "border-t-.6": index === 0 }, "inline-grid grid-cols-10 gap-6px items-end w-full border-white border-b-.6 py-2 cursor-pointer hover:bg-darkgreyHover"])}"><a${ssrRenderAttr("href", "/progetti/" + project.slug.current)} class="col-span-6"><p class="truncate">${project.title ?? ""}</p></a><p class="text-s col-start-7 col-span-3 truncate">${project.citta ?? ""}</p><p class="projectIndexArrow self-center col-start-10 col-span-1 text-right"></p></div>`);
        });
        _push(`<!--]--></div></main></div>`);
      }
    };
...
//# sourceMappingURL=project-index-B4VsaYAi.js.map
