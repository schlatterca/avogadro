...
import { u as useHead } from "./index-CCqbQxu4.js";
import { c as useNuxtApp, d as asyncDataDefaults, e as createError, b as useRuntimeConfig, f as defineStore } from "../server.mjs";
import { ref, shallowRef, toRef, getCurrentInstance, onServerPrefetch, unref, mergeProps, useSSRContext } from "vue";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value) {
        ...
        _push(`</figure><div id="snapContainer" class="flex snap-mandatory snap-x overflow-scroll absolute left-0 top-0 w-screen h-screen"><div id="empty" class="pic snap-start w-screen h-screen shrink-0"></div><div id="planimetria" class="w-66% ml-33% flex pic snap-end h-screen shrink-0 bg-lightgrey"><figure class="ml-17vw w-33% mt-50vh bg-lightgrey -translate-x-1/2 -translate-y-1/2">`);
        ...
        _push(`</figure><div class="m-auto w-33% ml-0 DM-Mono leading-tight">`);
        if (myData.value.title) {
          _push(`<p class="text-l uppercase">${myData.value.title ?? ""}</p>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (myData.value.description) {
          _push(`<p class="text-l mt-8">${myData.value.description ?? ""}</p>`);
        } else {
          _push(`<!---->`);
        }
        ...
        ssrRenderList(myImages.value.slides, (slide) => {
          _push(`<div class="${ssrRenderClass([
            ...
            "pic snap-start w-screen h-screen shrink-0 flex gap-20px items-end p-20px bg-lightgrey"
          ])}"><!--[-->`);
...
//# sourceMappingURL=_slug_-BFajKo88.js.map
