  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value) {
        ...
        ssrRenderList(unref(sortedProjects), (project, index) => {
          _push(`<a${ssrRenderAttr("href", "/progetti/" + project.slug.current)} class="${ssrRenderClass([{ "border-t-[1px]": index === 0 }, "border-b-[1px]"])}"><div class="singleproject inline-grid grid-cols-10 gap-6px items-end group w-full border-white py-2 cursor-pointer md:hover:bg-darkgreyHover"><p class="text-[.8rem] md:text-base col-span-6 truncate">${project.title ?? ""}</p><p class="text-[.6rem] md:text-s col-start-7 col-span-3 truncate">${project.citta ?? ""}</p><p class="projectIndexArrow self-center col-start-10 col-span-1 text-right opacity-0 md:group-hover:opacity-100 text-[15px] md:text-base"></p></div></a>`);
        });
...
//# sourceMappingURL=project-index-qjqoifea.js.map
